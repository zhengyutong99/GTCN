...
class GCNConv(MessagePassing):
    ...
    def __init__(self,
        ...
        self.weight = Parameter(torch.Tensor(in_channels, out_channels)).to(args.gpu_id)
    ...
    def forward(self, x, edge_index, edge_weight=None, drug_indices=None, cancer_indices=None):
        """"""
        # 確保所有輸入都在 GCN 的設備上
        ...
        # Extract drug-cancer pairs
        # drug_features = x[drug_indices]
        # cancer_features = x[cancer_indices]
    ...
    def message(self, x_j, norm):
        device = self.weight.device
        return norm.view(-1, 1) * x_j.to(device)

    def update(self, aggr_out):
        device = self.weight.device
        if self.bias is not None:
            aggr_out = aggr_out + self.bias.to(device)
        return aggr_out
